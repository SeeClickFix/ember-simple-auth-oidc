...
module("Unit | Authenticator | OIDC", function (hooks) {
  ...
  test("it supports sending custom parameters", function (assert) {
    ...
    assert.strictEqual(
      bodyWithRefresh,
      "redirect_uri=test%2Fredirect&grant_type=refresh_token&foo=bar&refresh_token=test-refresh-token",
    );
    ...
    assert.strictEqual(
      bodyWithoutRefresh,
      "redirect_uri=test%2Fredirect&grant_type=authorization_code&foo=bar&code=test-code",
    );
  });
});
